...
export default function ExerciseCategory({ category, categoryPoints, hasBeatdown = false }: ExerciseCategoryProps) {
  ...
  return (
    <div className="mb-3 rounded-xl overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700">
      <button type="button" onClick={() => setIsOpen((o) => !o)} className="w-full flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        <div className="flex items-center gap-3 text-left">
          <span className="text-xl">{config.emoji}</span>
          <div>
            <p className="font-bold text-sm text-f3navy dark:text-gray-100">
              {config.label}
            </p>
